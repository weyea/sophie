...
export let Actions = Type({
  ...
  replaceNode: {oldNode:Any, newNode:Any, path:Path, index: Number, placeHolder:Any},
  removeNode: {node:Any},
  sameNode: {node:Any},
...
export function diffNode (prev, next, path, index) {
  ...
  if (!isNull(prev) && isNull(next) || isNull(prev) && !isNull(next)) {
    return [replaceNode(prev, next, path, index)]
  }
  ...
  if (isThunk(next)) {
    ...
    if (isSameThunk(prev, next)) {
      ...
      changes.push(replaceNode(prev, next, path,index, placeholder))
    }
    return changes
  }
