...
export function diffChildren (previous, next, parentPath) {
  let { insertChild, updateChild, removeChild, moveChild,insertBefore, updateChildren } = Actions
  ...
  function effect (type, prev, next, pos) {
    ...
    switch (type) {
      ...
      case MOVE: {
        ...
        changes.push(moveChild(pos, next.item,prev.item, actions, nextPath))
        break
      }
      case REMOVE: {
...
export function diffNode (prev, next, path, index) {
  ...
  if (isThunk(next)) {
    ...
    if (isSameThunk(prev, next)) {
      ...
      changes.push(replaceNode(prev, next, path, undefined))
    }
    return changes
  }
